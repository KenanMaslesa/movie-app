...
export class SearchService {
  ...
  discover(mediaType='movie', page,genres,keywwords=''){
    return this.http.get(`https://api.themoviedb.org/3/discover/${mediaType}?api_key=${this.apiKey}&language=en-US&sort_by=popularity.desc&include_adult=false&page=${page}&with_genres=${genres}&without_keywords=${this.keywords}&with_keywords=${keywwords}`).pipe();
  }

  getKeywords(query){
    return this.http.get(`https://api.themoviedb.org/3/search/keyword?api_key=${this.apiKey}&query=${query}`).pipe();
  }
}
